...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- ANALYTICS: Track Page View (once per session) + live count updates ---
    const VIEW_SESSION_KEY = `ec_view_${CONFIG.eventId}`;

    const refreshViewCount = async () => {
        if (!_supabase || !CONFIG.eventId) return null;
    ...
    const trackPageView = async () => {
        try {
            ...
            const isRecoveryUrl = /[?&]_recover=/.test(window.location.search);
            const alreadyCounted = sessionStorage.getItem(VIEW_SESSION_KEY);

            if (!isRecoveryUrl && !alreadyCounted) {
                ...
                const { error } = await _supabase.from('page_views').insert([viewRow]);
                if (!error) sessionStorage.setItem(VIEW_SESSION_KEY, String(Date.now()));
            }
    ...
    const startViewCountLiveUpdates = () => {
        if (window._ecViewCountPoll) return;
        window._ecViewCountPoll = setInterval(refreshViewCount, 30000);
    };

    trackPageView();
    startViewCountLiveUpdates();
    initLiveViewerCount();
    ...
    const photogName = document.getElementById('footer-photographer-name');
    ...
    if (CONFIG.photographer) {
        ...
        const studioLabel = CONFIG.photographer.studio_name || '';
        const personLabel = studioLabel ? (CONFIG.photographer.name || '') : '';
        if (name) {
            if (studioLabel) {
                name.innerText = studioLabel;
                name.style.display = 'block';
            } else if (CONFIG.photographer.name) {
        ...
        if (photogName) {
            if (personLabel) {
                photogName.innerText = personLabel;
                photogName.style.display = 'block';
            } else {
                photogName.style.display = 'none';
            }
        }
        ...
        if (photogName) photogName.style.display = 'none';
        ...
        if (playerContainer) {
            ...
            const destroyHls = () => {
                ...
                if (video && video._dropHandlers) {
                    const dh = video._dropHandlers;
                    dh.clearStallTimer && dh.clearStallTimer();
                    video.removeEventListener('waiting', dh.onStallEvent);
                    video.removeEventListener('stalled', dh.onStallEvent);
                    video.removeEventListener('ended', dh.confirmStreamStatus);
                    video.removeEventListener('playing', dh.onResumeEvent);
                    video.removeEventListener('canplay', dh.onResumeEvent);
                ...
                resolveHlsPlaybackUrl(CONFIG.restreamerUrl)
                    .then((playbackUrl) => {
                            ...
                            if (typeof Hls !== 'undefined' && Hls.isSupported()) {
                                ...
                                // Stream status drop checker — only treated as a real
                                // interruption once buffering persists beyond a short
                                // grace period, since brief "waiting"/"stalled" events
                                // are normal for live HLS and were previously causing
                                // full player teardown/rebuild on every micro-stall.
                                let stallTimer = null;
                                ...
                                const onResumeEvent = () => clearStallTimer();

                                video.addEventListener('waiting', onStallEvent);
                                video.addEventListener('stalled', onStallEvent);
                                video.addEventListener('ended', confirmStreamStatus);
                                video.addEventListener('playing', onResumeEvent);
                                video.addEventListener('canplay', onResumeEvent);
                                video._dropHandlers = { onStallEvent, onResumeEvent, confirmStreamStatus, clearStallTimer };
                                    ...
                                    if (data.fatal) {
                                        switch (data.type) {
                                            case Hls.ErrorTypes.NETWORK_ERROR:
                                                if (!networkRetried) {
                                                    // hls.js can usually recover a transient network error
                                                    // (e.g. a single 404 on a segment that rolled off the
                                                    // sliding window) by just restarting the load — avoid a
                                                    // full player rebuild unless that also fails.
                                                    networkRetried = true;
                                                    console.warn("Network error, retrying load before full reconnect...");
                                                    try { hls.startLoad(); } catch (_) { /* fall through to rebuild */ }
                                                    setTimeout(() => { networkRetried = false; }, 15000);
                                                    break;
                                                }
...
if (wishesForm) {
    wishesForm.addEventListener('submit', async (e) => {
        ...
        const wishRow = { name, message, event_id: CONFIG.eventId };
        if (CONFIG.studioId) wishRow.studio_id = CONFIG.studioId;
...
(function initGuestPhotoWall() {
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const res  = await fetch('guest-photos/upload', { method: 'POST', body: fd });
