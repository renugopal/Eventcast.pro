// --- CONFIG ---
const WEDDING_DATE = new Date('April 12, 2026 10:30:00').getTime();
...
async function fetchWishes() {
    const { data, error } = await _supabase
        .from('wishes')
        .select('*')
        .eq('event_id', 'srineetha-dasaradh')
...
if (wishesForm) {
    wishesForm.addEventListener('submit', async (e) => {
        ...
        const { error } = await _supabase
            .from('wishes')
            .insert([{ name, message, event_id: 'srineetha-dasaradh' }]);
...
// Real-time
_supabase.channel('public:wishes_srineetha')
