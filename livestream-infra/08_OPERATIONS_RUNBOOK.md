...
VOD finalization above does not release the node capacity the event's Media Agent assignment occupies — finalization only publishes the VOD playlist against the Media Agent's own local state and never touches the `media_event_assignments` control-plane row. Capacity release is a separate, explicit operator step: `curl -X POST -H "Authorization: Bearer $MEDIA_NODE_PROVISIONING_SECRET" https://<admin-host>/internal/media/assignments/<event_id>/deactivate`. This call is idempotent and safe to repeat — a first successful call returns HTTP 200 with `outcome: "deactivated"`; every subsequent call for the same event returns HTTP 200 with `outcome: "already_inactive"`, never an error. This slice provides operator-triggered capacity release only — nothing calls this automatically when an event ends or is finalized, so an operator must trigger it explicitly as part of closing out an event if that node's capacity needs to be freed for a future assignment.
