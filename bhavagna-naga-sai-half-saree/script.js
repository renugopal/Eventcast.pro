...
function updateCountdown() {
    ...
    if (distance < 0) {
        wrapper.innerHTML = `<h3 style="color: var(--gold); font-family: 'Cinzel', serif;">The Ceremony is LIVE! 🎉</h3>`;
        return;
    }
...
if (wishesForm) {
    wishesForm.addEventListener('submit', async (e) => {
        ...
        const { error } = await _supabase
            .from('wishes')
            .insert([{ name, message, event_id: 'bhavagna-naga-sai' }]);
...
// Real-time
_supabase.channel('public:wishes_bhavagna')
