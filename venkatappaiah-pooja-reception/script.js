...
function updateCountdown() {
    ...
    if (distance < 0) {
        ...
        if (wrapper) wrapper.innerHTML = `<h3 style="color: var(--gold); font-family: 'Cinzel', serif;">The Reception is LIVE! 🎉</h3>`;
        return;
    }
...
async function fetchWishes() {
    ...
    const { data, error } = await _supabase
        .from('wishes')
        .select('*')
        .eq('event_id', 'venkatappaiah-poojasri')
...
if (wishesForm) {
    wishesForm.addEventListener('submit', async (e) => {
        ...
        const { error } = await _supabase
            .from('wishes')
            .insert([{ name, message, event_id: 'venkatappaiah-poojasri' }]);
...
// Real-time
_supabase.channel('public:wishes_venkatappaiah_poojasri')
