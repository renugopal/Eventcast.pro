...
**Superseded a final time (2026-08-15):** everything above about Milestone N being partially complete or having remaining work is now **DONE** — see "Completed Delivery Package — Remaining Milestone N VOD Lifecycle (2026-08-15)" below. Milestone N is fully COMPLETE / PASS for its approved V1 scope. **The next task for a fresh session is Milestone M (Super Admin Operations Console)** — entirely unstarted, requires its own fresh bounded-task scoping pass. Milestone O (legacy cutover) remains last, after Milestone M.
...
**Superseded (2026-08-15): all of the "remaining Milestone N work" named directly above is now DONE.** See "Completed Delivery Package — Remaining Milestone N VOD Lifecycle (2026-08-15)" below. Milestone N is now fully COMPLETE / PASS for its approved V1 scope. **The next project action is Milestone M (Super Admin Operations Console)** — unstarted, requiring its own fresh bounded-task scoping pass.

## Completed Delivery Package — Remaining Milestone N VOD Lifecycle (2026-08-15)

**COMPLETE / PASS for the approved V1 scope. Do not reopen any part of this.** Delivered on top of the already-COMPLETE/PASS DB foundation and production B2 rollout above (neither reopened): a real B2 playback-delivery path, replay-expiry behavior, and verified YouTube-fallback integration.

**B2 playback delivery.** The public render Worker gained an authenticated Worker-to-B2 read path (new `workers/render-event-page/src/b2playback.mjs`: exact Media Agent B2 key layout, manifest-relative-URI rewriting, and a hand-written AWS SigV4 signer using only Web Crypto — no new dependency). B2 credentials/host/keys never reach the browser. `isB2ReplayEligible()` fails closed on any missing evidence (`b2_finalized`, `finalization_generation`, `integrity_verified_at`, unexpired `retention_expires_at`) or unconfigured B2 — no B2 read credentials are provisioned anywhere today, so this resolves inactive in every current environment, same posture as the archival write side had before its own credentials were configured.

**Replay priority + fallback.** `renderEvent()`: Live → B2 replay (within retention) → legacy `vod_link`, unchanged byte-verbatim. `toProviderSafeRecordingView()` now reports `available` only with verified archive + unexpired retention + configured B2 read path. The Event Workspace Live tab's Recording card now shows real status instead of a placeholder.

**The missing YouTube-fallback verification decision — resolved by explicit user decision, then implemented.** V1 verification is **manual Super Admin attestation with an audit trail** — never a provider action, never an OAuth/API call (explicitly deferred to a future phase). New migration `eventcast-admin/supabase/migrations/0037_youtube_fallback_verification_rpc.sql` — **applied to the linked Supabase project on 2026-08-15; post-apply verification passed.** Adds no new column; one `SECURITY DEFINER` RPC requiring the caller be `super_admin`, requiring the supplied URL to equal the event's current `events.youtube_url` (the existing YTB-003 manual watch-link field is the sole candidate-URL authority — no second field introduced), and writing `youtube_fallback_url`/`youtube_fallback_verified` together with an atomic `platform_audit_log` row via the existing `0035` audit mechanism. New narrow route `POST /api/platform/events/[eventId]/youtube-fallback-verification` (`requireSuperAdmin()`-gated) — deliberately not the Super Admin Operations Console; no event lists, reports, or dashboards were built.

**Post-apply verification — PASS.** Migration history aligned local/remote through `0037`; RPC live with `SECURITY DEFINER`/correct `search_path`; `EXECUTE` grantees exactly `postgres` (owner, implicit) + `service_role` (no `anon`/`authenticated`/`PUBLIC`); the `event_recordings_youtube_fallback_requires_url` CHECK re-confirmed byte-identical; zero DDL in `0037` against any dependency table, so all pre-apply schema confirmations remain valid. No event/customer row mutated to test the RPC.

**Explicitly deferred, correctly out of scope:** R2 cleanup execution/reporting (Milestone M's surface — the eligibility predicate itself already existed from the `0035`/`0036` package and was reused, not reimplemented); OAuth/API-based YouTube verification (future phase); historical pre-`0004`/legacy-gap remediation.

**Validation.** Worker suite **64/64 PASS** (17 new tests, including a SigV4 signature cross-checked against an independent Node `crypto` reference implementation), `tsc --noEmit` clean. Full `eventcast-admin` Vitest **907/907 PASS** (5 new test files), `tsc --noEmit` unchanged at the same 59-line pre-existing/unrelated error baseline, zero new errors. See `CURRENT_STATE.md`/`WORKLOG.md` for full evidence.

**Milestone N (VOD / Retention Lifecycle) is now fully COMPLETE / PASS for its approved V1 scope.** No commit, push, deploy, Media Agent/SRS production change, secret access, or R2/B2 deletion occurred.

Continuity-file edits from this package were **not** committed or pushed.

## Milestone M — Super Admin Operations Console (COMPLETE / PASS, 2026-08-15)

**Read this before touching anything platform-related.** The Super Admin Operations Console is built, tested, and complete for its approved V1 scope. **Do not re-audit, re-design, or re-implement it.**

**Milestone N was not reopened.** The last trailing sentence of `WORKLOG.md`'s pre-Milestone-M history says Milestone N is "not fully complete". **That sentence is stale and superseded** — Milestone N reached COMPLETE / PASS for its approved V1 scope on 2026-08-15. It is retained unedited as history with a supersession note appended directly beneath it. Nothing in Milestone M re-audited, redesigned, re-applied, redeployed, or production-re-tested B2 archival, B2 playback, replay expiry, retention behavior, or YouTube fallback.

**No new schema.** A targeted reconciliation against Baseline V2.1 §15 and migrations through `0037` established that Milestone M needed **no migration at all**. Migration history is still `0001`–`0037`, unchanged. Nothing in `0035`–`0037`, the Media Agent B2 archival architecture, the B2 playback Worker, recording-transition semantics, retention-freeze semantics, or gap/provenance rules was modified.

**What exists now.** Fourteen `requireSuperAdmin()`-gated pages under `/platform` — Overview, Users & Studios (+ `/platform/studios/[studioId]`), All Events (+ `/platform/events/[eventId]`), Enabled Stream Assignments, SRS/Media Nodes, Templates, Media Operations, Support, Notifications, Security, Audit Log, Retention Policies, Storage, R2 Cleanup — backed by 17 platform API routes (13 new, 3 extended, plus the pre-existing retention/YouTube-fallback action routes). Shared layers: `src/lib/platformOperations.ts` (pure projections/derivations), `src/app/platform/_lib/platformClient.ts` (typed wrappers + `usePlatformResource`), `src/app/platform/_components/PlatformUI.tsx` (shared presentation, including the single `UnavailableNote` renderer).

**Rules that must hold for any future platform work.** `requireSuperAdmin()` is the first statement of every platform route — the service-role client bypasses RLS entirely, so nothing else protects cross-tenant data. Responses are explicit allowlists, never raw row spreads. Never project `stream_secret_hash`, `youtube_secret_reference`, `media_node_credentials`, `platform_users.mobile_number`, `.env` values, or raw authorization headers. Retention and YouTube-fallback actions must keep delegating to their existing RPCs (which pair the business write with its audit row atomically) — never write `event_recordings`, `platform_retention_policy`, or `studio_retention_overrides` directly. `youtube_fallback_verified = true` remains reachable only through `apply_youtube_fallback_verification` by an authorized Super Admin; no provider path may ever set it and no OAuth/API verification may be added. Reading private customer content requires a stated reason with the audit row written before disclosure. Where no authoritative source exists, return an explicit `{available: false, reason}` — never a synthesized number, never a silent zero.

**R2 cleanup — the one genuine remaining hard boundary in this area.** The console ships a complete non-destructive eligibility report and dry run reusing `isR2CleanupEligible()` verbatim, failing closed to zero candidate prefixes when the recording is ineligible or when no usable activation history proves which prefixes belong to the event. B2 objects are never a target and the route exposes no mutating verb. **Destructive execution was deliberately not implemented and needs explicit human decisions before it can be:** (1) the post-B2 grace duration before R2 objects may be removed is undefined; (2) the admin application holds no credential or endpoint for the media R2 bucket `eventcast-livestream-media` (the `R2_*` env vars address the separate images/guest-photo bucket; the media bucket is reachable only through the render Worker's `MEDIA_R2` binding), so a credential/execution channel must be chosen and provisioned; (3) the deletion scope — whole `events/{playbackId}/` prefix versus `media/` segments only, retaining `live/index.m3u8` and `vod/index.m3u8` — is undecided; (4) the Media Agent's node-side `EVENTCAST_R2_OBJECT_PREFIX` is unreadable from this application, so fully-qualified keys cannot be constructed here. No R2 or B2 object was listed, read, modified, or deleted.

**Deliberately not built, with reasons — do not "fix" these by inventing a mechanism.** Account suspend/restore, session termination, entitlement changes, trial extension: no suspension state exists, billing/entitlements are deferred by PLAN-007, and no session-revocation path exists. Template deployment/editing/version publishing: no such pipeline exists to drive; the surface is read-only by design. Platform-side Support reply: `support_ticket_messages` has no authorship-role column, so a Super Admin message would reach the provider's thread unattributed — an honest version needs an additive migration plus a provider-surface attribution change. Real outbound WhatsApp/SMS/email: no provider is integrated, and none was added.

**Validation.** New focused suites 77/77 PASS; directly-related regression suites 79/79 PASS across 14 files; full `eventcast-admin` Vitest **984/984 PASS across 88 files**; `npx tsc --noEmit` output byte-identical to the pre-change 59-line baseline (exit 2), zero new errors, no Milestone M file present. 40 files changed (32 new, 8 edited); no tracked file was modified. No Worker/Media Agent validation was run because no source change was needed there.

**Next direction: Milestone O — Controlled Legacy Retirement / Cutover.** It was **not** started here: no root-navigation change to V2, no legacy UI retirement, no Restreamer or GrapesJS cleanup, no duplicate preview/render path removal, no fake-analytics/stub retirement, no mass cleanup of historical customer directories or scratch tooling. Milestone O needs its own fresh bounded-task scoping pass. Also still unstarted and separate: real outbound WhatsApp/SMS/email integration, real technical stream telemetry, OAuth-connected YouTube channels and further SRS/Media Agent production activation, historical pre-`0004`/legacy-gap remediation, the legacy `photographers` RLS exposure, and R2 cleanup destructive execution.

**Superseded (2026-08-15): Milestone O has since been executed locally.** See "Completed Delivery Package — Milestone O" below. The paragraph directly above is retained unedited as history.

## Completed Delivery Package — Milestone O: Controlled Legacy Retirement / Cutover (2026-08-15)

**Status: LOCAL CUTOVER COMPLETE / PASS — production deployment pending explicit approval. Admin Baseline V2.1 implementation is locally complete.** A fresh session must **not** reopen, re-audit, redesign, or re-run this package.

**Root/provider cutover.** `/` now redirects to the V2 `/dashboard`. Login/signup and the Super Admin `/platform/*` boundary are unchanged; `AdminShell` still redirects unauthenticated visitors to `/login`. One deliberate security tightening: `/api/local-sync` was removed from middleware `PUBLIC_PREFIXES` and now returns 401.

**Retired (68 files):** the legacy Admin UI (16 components + `wallet-security.test.ts`, whose server-side guarantee stays covered by `billing-topup.test.ts`); GrapesJS and its supporting routes (23 `template-builder/*`, `admin/template-builder/page.tsx`, 3 `local-sync` routes, `localSyncPaths.ts`, 6 parked create-event wizard files); the Restreamer application paths (`lib/restreamer.ts`, `lib/security.ts`, `/api/stream/sign`, the three `/api/media/*` routes, `/api/cron/stream-health-monitor`) plus removal of the live `RestreamerClient` call from `/api/events/delete`; the duplicate GitHub-Raw preview path (`/api/events/preview`, `/api/template-asset`); `/api/events/generate`; and the fake-analytics/obsolete stubs (fabricated "Unique Reach", `/api/assets`, `/api/system/intelligence`) — each with its obsolete tests.

**Do not "finish the cleanup" on these — they are intentional compatibility.** `/portal/[slug]` is retained because the repository cannot prove no customer holds an out-of-band shared link; it was deliberately **not** converted to a 404 or redirect. `restreamerUrl`/`restreamerPlayer` in `weddingTemplateRenderer.ts` are a **live template contract** carrying the canonical R2/B2 HLS URL — renaming them breaks published customer pages. Also retained: the `restreamer_*` columns, `Toast.tsx`, `AuthShell.tsx`, the `eventContract.ts` legacy section, `/api/events/delete` + `/api/events/restore`, `/api/resolve-url`, the `scripts/` VOD tooling, `recover.js`, and `workers/hls-signer`.

**No migration; no dependency change.** History remains `0001`–`0037`; `package.json`/lockfile untouched. Newly-unused packages (`grapesjs*`, `@dnd-kit/*`, `uuid`/`@types/uuid`) were reported only, and remain a separate approval-gated task. `.github/workflows/cron-jobs.yml` retains only `sync-live-status` on the approved `*/15 * * * *` cadence; that job had previously been firing every 5 minutes too, because both jobs gated on `github.event_name == 'schedule'` without distinguishing which cron fired.

**Validation.** Focused 96/96; full Vitest **957/957 across 82 files**; `npm run build` PASS (route manifest contains no retired route); `npx tsc --noEmit` exit 2 with **zero new diagnostics** (59 → 14 lines, a strict subset of baseline); `git diff --check` exit 0; read-only post-deletion verification CLEAN with **zero executable references** across 21 scan patterns; authenticated browser smoke PASS with all 10 retired APIs returning 404 **when authenticated**. **Milestones N and M were not reopened.**
