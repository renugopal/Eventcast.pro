...
**COMPLETE / PASS, local implementation only.** This is the "Create Event / Admin UI-UX redesign" work explicitly named as deferred to the next workstream at Milestone O — Admin Baseline V2.1 itself is otherwise unaffected; this package sits outside the lettered A–O milestone sequence, same as the redesign was always understood to be separate from it.

`/events/new` is now one comprehensive, template-aware, mobile-friendly form instead of a 5-field stub with a fixed template label. `EventDraftInput`/`CanonicalEventRecord` widened with `venueMapLink`, `customTopTitle`, and a real `guestPhotoWallEnabled` input (all three map to pre-existing `public.events` columns — **no migration**). A new minimal `templateModules.ts` capability map drives which optional sections a given `template_id` shows (fail-closed, no fallback), currently populated for `wedding-template-01` only. Media, SEO thumbnail, Livestream YouTube watch-link, and Guest Memories Manual Approval are now stageable during Create Event and attached to the Draft after it exists, through the exact same post-creation routes the Event Workspace tabs already use — no new backend route. Livestream ingest activation stays deliberately excluded from Create Event's sequencing. `DraftEventForm.tsx` was refactored into a controlled component so one unified Cancel/Save Draft/Create Event action bar (sticky on mobile) governs the whole page. Partner logo upload was wired into the Partner Directory reusing the already-allow-listed `photographer_logo` R2 purpose.

A focused manual-verification pass (code-level only — no live authenticated browser session was available) found and fixed two real bugs: a thrown-exception path that could silently lose the reference to an already-created Draft mid-sequence (including during Publish itself), and a stale Partner-logo-upload error persisting across unrelated form sessions. A follow-up fix replaces the entire action row with a recovery-only row once a partial-success state exists, preventing a confusing duplicate-Draft-creation attempt.

Full repository Vitest: **975/975 passed across 85 files**; `npx tsc --noEmit` unchanged at the same pre-existing baseline, zero new errors. Full evidence: `WORKLOG.md` ("2026-09-18 — Create Event Redesign") and `CURRENT_STATE.md` ("Completed Delivery Package — Create Event Redesign"). No commit, push, deploy, migration, or secret action occurred. **Do not re-audit or re-implement this package.**

**Correction (2026-09-19):** the mobile sticky action bar claimed above was not actually sticky — static verification alone had missed that `.ec-main` never generates its own scrollbar on mobile (the document/window scrolls instead), so `position: sticky` had no real scrolling ancestor to engage with. Fixed narrowly (`.ec-create-event-actions` → `position: fixed` with `left/right: 0`, mobile-only) and live-verified at 375px via real authenticated browser testing: pinned at every scroll position including max-scroll, no permanent content occlusion, no horizontal overflow, desktop unaffected, partial-success recovery bar confirmed identical. Tests/type check unchanged after the fix. See `WORKLOG.md` ("2026-09-19 — Correction: mobile sticky action bar was not actually sticky") for full detail.

### Completed delivery package: Create Event Premium UI/UX Redesign (2026-09-19)

**COMPLETE / PASS, local implementation only, presentation-only.** A separate package on top of the Create Event Redesign above (not reopened) — visual/UX only, explicitly scoped to keep every piece of business logic (Draft/attachment/Publish sequencing, validation, Partner Credits, uploads, template-capability architecture, partial-success recovery, the mobile fixed action bar) unchanged.

`/events/new` gained a two-column desktop layout (~70/30, ≥1024px) — premium section cards with real-validation status pills (Required/Complete/Optional, using the exact same checks `isDraftEventFormValid` already computes), a card-based Template selector (unchanged `listCreatableTemplates()`/fail-closed architecture), accordion-styled optional modules with drop-zone upload surfaces, and a new sticky, live-updating **Event Summary** rail (`EventSummaryRail.tsx`, read-only, derives from existing page state only — never a second source of truth). A header progress chip shows "N of 4 required sections ready." Visibility is now two keyboard-accessible selectable cards (`aria-pressed`). Mobile (375px): single column, the rail becomes a compact collapsible summary, the previously-verified fixed action bar untouched. All new CSS reuses existing tokens/patterns (`.ec-plan-card`, `.ec-toggle`, the `fade-slide-up` keyframe) — no new dependency.

**A real bug found and fixed, live-verified:** narrow-mobile (375px) section-header status pills were wrapping onto their own row even when the title alone had room — root cause was `flex: 1 1 auto` on the heading block, whose `flex-basis: auto` makes flex-wrap eligibility get computed from the *unwrapped* subtitle text width before `flex-shrink` ever applies. Fixed by changing the basis to `0%` on the heading and its inner text wrapper, plus `flex-shrink: 0` on the trailing pill/chevron group. Verified live across every section at 375px and re-confirmed unaffected at 1400px desktop.

**A real dev-tooling gotcha, recorded for future sessions:** two CSS-only edits were not reflected by the running Turbopack dev server despite correct source and "✓ Compiled" logs — only stopping the server, deleting `eventcast-admin/.next`, and restarting cleared it. Not an application defect.

Full repository Vitest: **975/975 passed across 85 files**; `npx tsc --noEmit` unchanged at the same pre-existing baseline, zero new errors. Full live authenticated browser verification at desktop and mobile, including a real Create Event → Publish flow and a real partial-success recovery trigger. Full evidence: `WORKLOG.md` ("2026-09-19 — Create Event Premium UI/UX Redesign") and `CURRENT_STATE.md` ("Completed Delivery Package — Create Event Premium UI/UX Redesign"). No commit, push, deploy, migration, or secret action occurred. **Do not re-audit or re-implement this package.**

### Completed delivery package: Event Lifecycle Management / Permanent Delete / Draft Expiry (2026-09-19)

**PRODUCTION ROLLOUT COMPLETE / PASS.** Migration `0038` applied and live-verified; committed, built, deployed, and promoted to 100% production traffic on `eventcast-admin-worker` (Worker version `2d69e761-9fd9-440d-9ff9-2fcf3c92dfe1`); the automated cron sweep live-verified in GitHub Actions (run `35444188160`). Full detail: `CURRENT_STATE.md` and `WORKLOG.md`. **Do not re-audit or re-implement.**
...
**CLOSED / PASS, production database.** Closes the security finding flagged unresolved since Milestone F (2026-08-10) — see Milestone F's own note below. Migration `eventcast-admin/supabase/migrations/0039_photographers_rls_lockdown.sql` (two `DROP POLICY IF EXISTS` statements only, mirroring migrations `0022`/`0028`) dropped the two undocumented broad policies `Admin full access on photographers` and `Public can view photographers`; the four canonical `0003` tenant-scoped policies (`photographers_select_policy`/`_insert_policy`/`_update_policy`/`_delete_policy`) were confirmed present and are unchanged. Applied via `supabase db push --linked --yes` and post-apply verified (`pg_policies` re-query: exactly 4 policies remain, byte-identical definitions). New focused test `tests/contract/photographers-rls-cleanup.test.ts` — 8/8 PASS. No Worker/renderer/application code change was required. See `CURRENT_STATE.md` and `WORKLOG.md` for full evidence. **Do not re-audit, re-design, or re-apply this — it is closed.**
...
The legacy `photographers` RLS / authorization cleanup previously listed here is now CLOSED / PASS (2026-09-22) — see "Completed delivery package: Legacy `photographers` RLS lockdown" above. Do not list it as open work again.
