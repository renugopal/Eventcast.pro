-- ============================================================
-- ARCHIVED / HISTORICAL — NOT AN ACTIVE MIGRATION.
--
-- This file is the preserved, byte-identical original SQL body of
-- `eventcast-admin/supabase/migrations/0019_livestream_control_plane.sql`
-- as it existed before that active migration file was replaced with an
-- intentional no-op supersession marker.
--
-- This original design was NEVER applied to the production remote
-- database (see `eventcast-admin/supabase/migrations/0019_livestream_control_plane.sql`
-- and `livestream-infra/supabase/migrations/README.md`, "Validation commands":
-- runtime execution was deferred, only static SQL review was performed).
-- It was superseded by the narrower, reconciled schema in
-- `eventcast-admin/supabase/migrations/0020_media_agent_assignments.sql`
-- (for `media_nodes`) and `0021_media_node_auth.sql` (node credentials and
-- replay-nonce tables).
--
-- DO NOT APPLY THIS FILE. It is kept here only as historical design
-- documentation of the original, wider control-plane schema proposal
-- (media_nodes with a different column set, plus stream_sessions,
-- media_jobs, event_state_transitions, and additive columns on events).
-- If any environment is ever found to already contain stream_sessions,
-- media_jobs, event_state_transitions, or this original wider media_nodes
-- shape, migration application must stop for manual reconciliation rather
-- than applying or reconciling against this file automatically.
-- ============================================================
